...
cat $NPDES_DIR/NPDES_CS_VIOLATIONS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.npdes_cs_violations FROM STDIN WITH CSV HEADER;'
...
cat $NPDES_DIR/NPDES_FORMAL_ENFORCEMENT_ACTIONS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.npdes_formal_enforcement_actions FROM STDIN WITH CSV HEADER;'
...
cat $NPDES_DIR/NPDES_INFORMAL_ENFORCEMENT_ACTIONS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.npdes_informal_enforcement_actions FROM STDIN WITH CSV HEADER;'
...
cat $NPDES_DIR/NPDES_NAICS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.npdes_naics FROM STDIN WITH CSV HEADER;'
...
cat $NPDES_DIR/NPDES_PS_VIOLATIONS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.npdes_ps_violations FROM STDIN WITH CSV HEADER;'
...
cat $NPDES_DIR/NPDES_QNCR_HISTORY.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.npdes_qncr_history FROM STDIN WITH CSV HEADER;'
...
cat $NPDES_DIR/NPDES_SICS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.npdes_sics FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_DEFENDANTS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_defendants FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_ENFORCEMENTS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_enforcements FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_RELIEF_SOUGHT.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_relief_sought FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_PENALTIES.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_penalties FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_LAW_SECTIONS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_law_sections FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_FACILITIES.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_facilities FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_MILESTONES.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_milestones FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_POLLUTANTS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_pollutants FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_ENFORCEMENT_CONCLUSIONS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_enforcement_conclusions FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_ENFORCEMENT_CONCLUSION_POLLUTANTS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_enforcement_conclusion_pollutants FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_ENFORCEMENT_CONCLUSION_COMPLYING_ACTIONS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_enforcement_conclusion_complying_actions FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_ENFORCEMENT_CONCLUSION_FACILITIES.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_enforcement_conclusion_facilities FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_ENFORCEMENT_CONCLUSION_SEP.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_enforcement_conclusion_sep FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_ENFORCEMENT_CONCLUSION_DOLLARS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_enforcement_conclusion_dollars FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_REGIONAL_DOCKETS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_regional_dockets FROM STDIN WITH CSV HEADER;'
...
cat $CASE_DIR/CASE_RELATED_ACTIVITIES.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.fec_related_activities FROM STDIN WITH CSV HEADER;'
...
cat $AIR_DIR/ICIS-AIR_FACILITIES.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.air_facilities FROM STDIN WITH CSV HEADER;'
...
cat $AIR_DIR/ICIS-AIR_PROGRAMS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.air_programs FROM STDIN WITH CSV HEADER;'
...
cat $AIR_DIR/ICIS-AIR_PROGRAM_SUBPARTS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.air_program_subparts FROM STDIN WITH CSV HEADER;'
...
cat $AIR_DIR/ICIS-AIR_POLLUTANTS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.air_pollutants FROM STDIN WITH CSV HEADER;'
...
cat $AIR_DIR/ICIS-AIR_FCES_PCES.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.air_fces_pces FROM STDIN WITH CSV HEADER;'
...
cat $AIR_DIR/ICIS-AIR_STACK_TESTS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.air_stack_tests FROM STDIN WITH CSV HEADER;'
...
cat $AIR_DIR/ICIS-AIR_TITLEV_CERTS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.air_titlev_certs FROM STDIN WITH CSV HEADER;'
...
cat $AIR_DIR/ICIS-AIR_FORMAL_ACTIONS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.air_formal_actions FROM STDIN WITH CSV HEADER;'
...
cat $AIR_DIR/ICIS-AIR_INFORMAL_ACTIONS.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.air_informal_actions FROM STDIN WITH CSV HEADER;'
...
cat $AIR_DIR/ICIS-AIR_HPV_HISTORY.csv | psql -v ON_ERROR_STOP=1 -c '\COPY icis.air_hpv_history FROM STDIN WITH CSV HEADER;'
