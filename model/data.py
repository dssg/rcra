...
from drain.data import FromSQL, index_as_series
...
class EpaData(Step):
    EXCLUDE = {'formal_enforcement_epa', 'formal_enforcement_state', 'formal_enforcement',
                'min_formal_enforcement_date_epa', 'min_formal_enforcement_date_state', 
                'min_formal_enforcement_date',
    ...
    AUX = ['evaluation', 'region', 'state', 'handler_received', 'handler_age']

    def __init__(self, month, day, year_min=2007, year_max=2015, outcome_years=1, investigations_drop_lists=True, **kwargs):
        Step.__init__(self, month=month, day=day, year_min=year_min, year_max=year_max, 
                outcome_years=outcome_years, 
                investigations_drop_lists=investigations_drop_lists, **kwargs)
        ...
        self.aggregators = { 'investigations': aggregations.investigations(dates),
                             'handlers': aggregations.handlers(dates) }

        sql_vars = { 'doy' : '%02d%02d' % (month, day), 
                'date_min' : date(year_min, month, day), 'date_max' : date(year_max, month, day) }

        self.inputs = [FromSQL(query="""
select * from output.facility_years{doy} 
where date between '{date_min}' and '{date_max}'""".format(**sql_vars), parse_dates=['date'])] + self.aggregators.values()
        self.inputs_mapping = ['X']

    def run(self, X, *args, **kwargs):
        X = self.aggregators['investigations'].join(X)
        X = self.aggregators['handlers'].join(X)
